...
normalized_XYZ_values = XYZ_values * N_values
...
dot_product = dot(v1, v2) / (lenV(v1) * lenV(v2))
...
R = identity_matrix + (math.sin(angle_radians) * As) + (1 - math.cos(angle_radians)) * (As ** 2)

# 旋转 Oklab 数值
NAA_value = []
for value in OkLab_values:
    if np.array_equal(value, OkLab_values[6]):
        NAA_value.append(value)
        continue
    v = value - OkLab_values[0]
    newPoint = np.dot(R, v) + OkLab_values[0]
